                      ...
                      const Vector<ColorI>* alarmVLights)
{
   ...
   renderStaticMeshes(true, true, instanceHandle);
}

void Interior::renderStaticMeshes(bool texture, bool lightmap, LM_HANDLE instancelmhandle)
{
   // Lightmaps
   if (lightmap)
   {
   ...
   for (U32 i = 0; i < mStaticMeshes.size(); i++)
   {
      ...
      glScalef(mesh->scale.x, mesh->scale.y, mesh->scale.z);
		...
		mesh->render(true, true, lightmap, getLMHandle(), instancelmhandle);
...
void Interior::renderAsShape()
{
   ...
   renderStaticMeshes(true, false, 0);
}
