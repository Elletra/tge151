...
void EditGeometry::dumpGameEntityToRuntime(Interior*            /*pRuntime*/,
                                          ...
                                          GameEntity*   pEntity)
{
   AssertWarn(mWorldEntity->mDetailNumber == 0, "Error, GameEntities only go on the 0 detail level!");
   mWorldEntity->mDetailNumber = 0; // pss fix
   EditInteriorResource* pEditResource = dynamic_cast<EditInteriorResource*>(pResource);
                                     ...
                                     DoorEntity*       pEntity)
{
   ...
   for(U32 i = 0; i < pEntity->mWayPoints.size(); i++)
   {
      InteriorPathFollower::WayPoint wp = *pEntity->mWayPoints[i];
      pChild->mWayPoints.push_back(wp);
   }
...
{
   // Really slow.  ah, well...
   // MDF - made it even slower - woot!
   Box3F   boundBox(Point3F(1e10, 1e10, 1e10), Point3F(-1e10, -1e10, -1e10), true);
   Point3F centroid(0, 0, 0);

   for (U32 i = 0; i < pRuntime->mWindings.size(); i++)
   {
      Point3F& pt = pRuntime->mPoints[pRuntime->mWindings[i]].point;

      boundBox.min.setMin(pt);
      boundBox.max.setMax(pt);
   ...
   centroid /= F32(pRuntime->mWindings.size());
   ...
   for (U32 i = 0; i < pRuntime->mWindings.size(); i++)
   {
      ...
      F32 lenSQ = (pt - centroid).lenSquared();
      if (lenSQ > maxRSq)
         maxRSq = lenSQ;
   }
